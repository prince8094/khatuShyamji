...
export function ProfileScreen({
  ...
  const [loadingGoogleLink, setLoadingGoogleLink] = useState(false)
  ...
  const handleLinkGoogle = async () => {
    ...
    try {
      ...
      alert("Failed to link Google account: " + (err.message || err))
    } finally {
      setLoadingGoogleLink(false)
    }
  }
  ...
  const [editDob, setEditDob] = useState(displayUser.dob || getTodayStr())
  ...
  const handleCancel = () => {
    ...
    setEditDob(displayUser.dob || getTodayStr())
    ...
    <div className="relative space-y-5 pb-10">
      ...
      {!isEditing ? (
        <>
          ...
          {displayUser.phone.startsWith("+99") && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-semibold text-amber-800 space-y-1">
              <div className="flex items-center gap-2">
                <Icon name="AlertTriangle" className="size-4 shrink-0 text-amber-700" />
                <span className="font-bold">
                  {lang === "hi" ? "प्रोफ़ाइल अपडेट आवश्यक है" : "Profile Action Required"}
                </span>
              </div>
              <p className="font-medium text-amber-700 leading-relaxed">
                {lang === "hi" 
                  ? "आपने Google से लॉग इन किया है। कृपया मंदिर सेवाओं और बुकिंग के लिए अपना वास्तविक 10-अंकों का मोबाइल नंबर अपडेट करें।" 
                  : "You logged in with Google. Please click 'Edit Profile' below to update your real 10-digit mobile number for bookings."}
              </p>
            </div>
          )}
          ...
          <section className="overflow-hidden rounded-3xl border border-border bg-card p-5 shadow-sm space-y-3">
            <h4 className="font-heading text-sm font-bold text-foreground flex items-center gap-2">
              ...
              {lang === "hi" ? "लिंक्ड अकाउंट्स" : "Linked Accounts"}
            </h4>
            <div className="flex items-center justify-between p-3 rounded-2xl border border-secondary bg-secondary/20">
              <div className="flex items-center gap-2">
              ...
              {identities.some(i => i.provider === 'google') ? (
                <span className="flex items-center gap-1 text-xs font-bold text-green-600 bg-green-50 px-2.5 py-1 rounded-full border border-green-200">
                  ...
                  {lang === "hi" ? "लिंक्ड है" : "Linked"}
                </span>
              ) : (
                <button
                  onClick={handleLinkGoogle}
                  disabled={loadingGoogleLink}
