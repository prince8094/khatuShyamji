-- KhatuShyamji Smart Pilgrim Management System - Supabase Init Migration
...
-- ────────────────────────────────────────────────────────────────────────────
-- 4. Custom Database Helper Functions
-- ────────────────────────────────────────────────────────────────────────────

-- Custom auth loader helper to sync newly signed-up users automatically
...
-- Trigger config for on-demand profile provisioning
CREATE OR REPLACE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- STANDALONE pilgrim creator helper function
...
BEGIN
    ...
    -- Insert mock/actual credentials
...
CREATE POLICY devices_all ON public.device_tokens FOR ALL USING (auth.uid() = profile_id OR public.is_admin());
CREATE POLICY notify_select ON public.notifications FOR SELECT USING (is_broadcast = true OR public.is_admin());
CREATE POLICY notify_all ON public.notifications FOR ALL USING (public.is_admin());
CREATE POLICY recipients_all ON public.notification_recipients FOR ALL USING (auth.uid() = profile_id OR public.is_admin());
CREATE POLICY logs_all ON public.notification_logs FOR ALL USING (public.is_admin());
...
-- ────────────────────────────────────────────────────────────────────────────
-- 6. Storage Buckets Creation & RLS Policies
-- ────────────────────────────────────────────────────────────────────────────
...
-- RLS policies for storage buckets objects
...
CREATE POLICY "Owner All for profile-photos" ON storage.objects
  FOR ALL TO authenticated USING (bucket_id = 'profile-photos' AND (owner = auth.uid()::text OR public.is_admin()));

-- ────────────────────────────────────────────────────────────────────────────
-- 7. Seed Master Configuration Data
-- ────────────────────────────────────────────────────────────────────────────

-- Add default operational roles
...
-- Insert placeholder super-admin (Nand Kumar)
...
-- Map admin to super-admin and commerce roles
...
-- Seed infrastructure parking blocks
